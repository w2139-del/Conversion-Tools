...
# ★ KMLを直接文字列で生成する関数（Google Maps完全対応）
def build_kml(res_data, kml_export_type, drawn_data):
    ...
    if "ポイント" in kml_export_type:
        for _, r in res_data.iterrows():
            name = str(r['点名']).replace('&', '&amp;').replace('<', '&lt;').replace('>', '&gt;')
      ...
      <Point>
        <coordinates>{r['経度']},{r['緯度']},{r['楕円体高']}</coordinates>
      </Point>
    </Placemark>""")
...
if 'result' in st.session_state:
    ...
    # ★ simplekml廃止 → 直接KML文字列を生成
    drawn_data = st.session_state.get('drawn_data', None)
    kml_bytes = build_kml(res_data, kml_export_type, drawn_data)
